...
- **Row rendering** uses the Blizzard XML mixin pattern: `LootDisplayRowTemplate.xml` composes `LootDisplayRowMixin` plus seven `RLF_Row*Mixin` sub-mixins. Row sub-mixin globals are prefixed `RLF_` to avoid `_G` collisions. Text sizing uses native engine truncation (`SetWidth` + `SetWordWrap(false)`), not manual width math.
