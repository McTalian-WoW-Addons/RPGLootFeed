--@strip-comments@
---Translator ZamestoTV v1.22.3, 1.27.2, 1.28.0, 1.32.0
---Translator Карнажж v1.23.0, v1.25.0, v1.26.0, v1.27.0
---@type string, G_RLF
local _, G_RLF = ...

local L = LibStub("AceLocale-3.0"):NewLocale(G_RLF.localeName, "ruRU")
if not L then
	return
end

--- Place newest translations/locale keys at the top, wrapped in --#region and --#endregion for the version number that they were added in.
--- You may translate these comments, but do not translate "region" or "endregion" as they are used by the localization tool to determine where to place the translations.
--- To add translations, simply uncomment the line(s) and replace the English text after the equal sign (=) with the translated value.

--#region 1.34.0
-- L["Automatic"] = "Automatic"
-- L["Disable Slug"] = "Disable Slug"
-- L["DisableSlugDesc"] = "Turn off Slug rendering for this frame's outlined text, going back to the standard outline."
-- L["Drain Direction"] = "Drain Direction"
-- L["DrainDirectionDesc"] = "Which direction the timer bar empties. Right to Left is the default countdown feel."
-- L["Enable Timer Bar"] = "Enable Timer Bar"
-- L["EnableTimerBarDesc"] = "Show a timer bar on loot rows showing how long until they fade out."
-- L["Fade Out Timer Bar"] = "Fade Out Timer Bar"
-- L["FadeOutTimerBarDesc"] = "Configure the timer bar that shows how long until a row fades out."
-- L["Icon Skin"] = "Icon Skin"
-- L["IconSkinDesc"] = "Choose which addon skins the loot row icons. Automatic uses whichever supported addon you have loaded. Square crops the icon border without needing any other addon."
-- L["IconSkinUnavailableWarning"] = "That icon skin is not available because the addon that provides it is not loaded."
-- L["Left to Right"] = "Left to Right"
-- L["NotInstalledSuffix"] = " (not installed)"
-- L["Right to Left"] = "Right to Left"
-- L["SlugFlagNote"] = "Outlines are rendered through the Slug library for crisper glyphs at any size. It has no effect while Use Font Objects is on."
-- L["Square"] = "Square"
-- L["Timer Bar Alpha"] = "Timer Bar Alpha"
-- L["Timer Bar Color"] = "Timer Bar Color"
-- L["Timer Bar Height"] = "Timer Bar Height"
-- L["Timer Bar Y Offset"] = "Timer Bar Y Offset"
-- L["TimerBarAlphaDesc"] = "Transparency of the timer bar (0 = invisible, 1 = fully opaque)."
-- L["TimerBarColorDesc"] = "Color of the timer bar fill."
-- L["TimerBarHeightDesc"] = "Height of the timer bar in pixels."
-- L["TimerBarYOffsetDesc"] = "Vertical offset of the timer bar in pixels. Use a positive value to move it above the row border."
-- L["Close History"] = "Close History"
-- L["Double Scroll Required"] = "Double Scroll Required"
-- L["DoubleScrollRequiredDesc"] = "Require two scroll events within the time window to activate/deactivate history, reducing accidental triggers."
-- L["Drain Direction"] = "Drain Direction"
-- L["DrainDirectionDesc"] = "Which direction the timer bar empties. Right to Left is the default countdown feel."
-- L["Enable Loot Rolls in Feed"] = "Enable Loot Rolls in Feed"
-- L["Enable Scroll Wheel History Activation"] = "Enable Scroll Wheel History Activation"
-- L["Enable Timer Bar"] = "Enable Timer Bar"
-- L["EnableLootRollsDesc"] = "Enable loot rolls in the loot feed"
-- L["EnableScrollWheelActivationDesc"] = "When enabled, scrolling down on the loot frame will activate loot history. Scroll up at the top to deactivate."
-- L["EnableTimerBarDesc"] = "Show a timer bar on loot rows showing how long until they fade out."
-- L["Fade Out Timer Bar"] = "Fade Out Timer Bar"
-- L["FadeOutTimerBarDesc"] = "Configure the timer bar that shows how long until a row fades out."
-- L["Left to Right"] = "Left to Right"
-- L["Loot Rolls Config"] = "Loot Rolls Config"
-- L["Right to Left"] = "Right to Left"
-- L["Roll Button Size"] = "Roll Button Size"
-- L["RollButtonSizeDesc"] = "Size of the roll action buttons (Need, Greed, Pass) in pixels."
-- L["Scroll Wheel Target Anchor"] = "Scroll Wheel Target Anchor"
-- L["Scroll Wheel Target Height"] = "Scroll Wheel Target Height"
-- L["Scroll Wheel Target Width"] = "Scroll Wheel Target Width"
-- L["Scroll Wheel Target X Offset"] = "Scroll Wheel Target X Offset"
-- L["Scroll Wheel Target Y Offset"] = "Scroll Wheel Target Y Offset"
-- L["Scroll Window (ms)"] = "Scroll Window (ms)"
-- L["ScrollWheelTargetAnchorDesc"] = "Which corner or edge of the detection area to anchor to the matching point on the loot frame."
-- L["ScrollWheelTargetHeightDesc"] = "Override the height of the scroll-wheel detection area in pixels. Set to 0 to auto-size to the loot frame height."
-- L["ScrollWheelTargetWidthDesc"] = "Override the width of the scroll-wheel detection area in pixels. Set to 0 to auto-size to the loot frame width."
-- L["ScrollWheelTargetXOffsetDesc"] = "Horizontal offset (in pixels) of the detection area relative to its anchor point on the loot frame."
-- L["ScrollWheelTargetYOffsetDesc"] = "Vertical offset (in pixels) of the detection area relative to its anchor point on the loot frame."
-- L["ScrollWindowDesc"] = "Time window in milliseconds within which two scroll events must occur to trigger history activation."
-- L["Show Border on Hover"] = "Show Border on Hover"
-- L["ShowScrollTargetBorderOnHoverDesc"] = "When enabled, hovering over the scroll-wheel detection area will show a cyan border around it. Useful for verifying its position and size."
-- L["Timer Bar Alpha"] = "Timer Bar Alpha"
-- L["Timer Bar Color"] = "Timer Bar Color"
-- L["Timer Bar Height"] = "Timer Bar Height"
-- L["Timer Bar Y Offset"] = "Timer Bar Y Offset"
-- L["TimerBarAlphaDesc"] = "Transparency of the timer bar (0 = invisible, 1 = fully opaque)."
-- L["TimerBarColorDesc"] = "Color of the timer bar fill."
-- L["TimerBarHeightDesc"] = "Height of the timer bar in pixels."
-- L["TimerBarYOffsetDesc"] = "Vertical offset of the timer bar in pixels. Use a positive value to move it above the row border."
--#endregion

--#region 1.33.0
-- L["Background Override"] = "Background Override"
-- L["BackgroundOverrideDesc"] = "Override row background colors for this loot feed type while using the frame's selected background mode."
-- L["Currency Deny List"] = "Currency Deny List"
-- L["Enable Background Color Override"] = "Enable Background Color Override"
-- L["EnableBackgroundColorOverrideDesc"] = "If enabled, rows from this loot feed use the override colors below instead of the frame's global background colors."
-- L["Free Position Loot History Tab"] = "Free Position Loot History Tab"
-- L["FreePositionLootHistoryTabDesc"] = "Detach the loot history tab from the feed corner and position it anywhere on screen using the X and Y offsets."
-- L["Ignore Currency IDs"] = "Ignore Currency IDs"
-- L["IgnoreCurrencyIDsDesc"] = "Enter a comma-separated list of currency IDs to ignore in the Currency Feed"
-- L["IgnoreItemIDsItemLootDesc"] = "Enter a comma-separated list of item IDs to ignore in the Item Loot Feed"
-- L["Item Loot Deny List"] = "Item Loot Deny List"
-- L["Loot History Tab Size"] = "Loot History Tab Size"
-- L["Loot History Tab X Offset"] = "Loot History Tab X Offset"
-- L["Loot History Tab Y Offset"] = "Loot History Tab Y Offset"
-- L["LootHistoryTabSizeDesc"] = "Set the size of the loot history tab button in pixels."
-- L["LootHistoryTabXOffsetDesc"] = "Adjust the tab's horizontal position. In free-position mode, this is an absolute offset from screen center."
-- L["LootHistoryTabYOffsetDesc"] = "Adjust the tab's vertical position. In free-position mode, this is an absolute offset from screen center."
--#endregion

--#region 1.32.0
L["Disable Mouse Interactions In Combat"] = "Отключить взаимодействие мышью в бою"
L["DisableMouseInCombatDesc"] = "Если включено, лента добычи не будет реагировать на нажатия мыши во время боя. Это позволит вращать камеру и использовать другие функции мыши 'сквозь' панель."
L["FMT_APPEARANCE_DESC"] = "Настройка внешнего вида окна добычи '%s': положение, размер, стиль и анимация."
L["FMT_DELETE_FRAME"] = "Удалить окно '%s'"
L["FMT_FRAME_DESC"] = "Настройка внешнего вида окна добычи '%s' и выбор отображаемых в нем данных."
L["FMT_LOOT_FEEDS_DESC"] = "Включить и настроить отдельные элементы ленты добычи, отображаемые в окне '%s'."
L["GeneralDesc1.32"] = "Общие настройки и быстрые действия, применяемые ко всем окнам добычи."
L["Interactions"] = "Взаимодействие"
L["InteractionsDesc"] = "Настройка того, как лента добычи реагирует на действия мыши."
L["Lock Row Position On Hover"] = "Фиксировать строку при наведении"
L["LockRowPositionOnHoverDesc"] = "Если включено, наведение курсора на строку зафиксирует её на месте, чтобы она не сдвигалась при исчезновении других строк из ленты."
L["Repositioning Animation"] = "Анимация перемещения"
L["Repositioning Duration"] = "Длительность перемещения"
L["RepositioningAnimationDesc"] = "Настройка анимации строк при их перемещении на новые позиции после исчезновения других элементов."
L["RepositioningDurationDesc"] = "Длительность анимации перемещения строк в секундах. Чем меньше значение, тем быстрее анимация; минимум (0.05 сек) делает сдвиг почти мгновенным."
--#endregion

--#region 1.31.1
L["AHSourceChangeConfirmFmt"] = "Ваш сохраненный источник аукциона '%s' сейчас не загружен. Изменение этого параметра перезапишет ваши настройки. Вы уверены?"
L["AHSourceUnavailableFmt"] = "%s (недоступно)"
L["PricesModeChangeConfirmFmt"] = "Ваш сохраненный режим цен '%s' требует аддон аукциона, который сейчас не загружен. Изменение этого параметра перезапишет ваши настройки. Вы уверены?"
--#endregion

--#region 1.31.0
L["DismissRecovery"] = "Отклонить и очистить"
L["RestoreOldSettings"] = "Восстановить старые настройки"
L["Settings Recovery"] = "Восстановление настроек"
L["SettingsRecoveryBody"] = "RPGLootFeed обнаружил настройки версии ниже %s, которые не были корректно перенесены при обновлении до %s. Ваши оригинальные настройки сохранены, и их можно восстановить ниже. Вы также можете закрыть это уведомление, если уже настроили аддон заново."
L["SettingsRecoveryDesc"] = "Обнаружены устаревшие настройки (версии ниже %s)."
L["SettingsRecoveryDetailsLabel"] = "Разница в настройках - старое значение -> текущее значение (скопируйте для справки):"
L["SettingsRecoveryNoChanges"] = "Различий между вашими старыми настройками и текущими значениями по умолчанию не обнаружено."
L["SettingsRecoverySummaryHdr"] = "Список различий между вашими старыми настройками и текущими значениями:"
--#endregion

--#region 1.30.2
L["Main"] = "Основное"
L["MigrationError"] = "Ошибка миграции (v%d):"
--#endregion

--#region 1.30.0
L["+ New Frame"] = "+ Новое окно"
L["Add Frame"] = "Добавить окно"
L["AddFrameDesc"] = "Создать новое окно для отображения добычи. Максимум 5 окон."
L["Appearance"] = "Внешний вид"
L["Chat Filters"] = "Фильтры чата"
L["ChatFiltersDesc"] = "Скрывать сообщения о добыче в окнах чата."
L["Currency"] = "Валюта"
L["DeleteFrameConfirm"] = "Удалить это окно? Все его настройки будут утеряны."
L["Experience"] = "Опыт"
L["Feed Display"] = "Отображение ленты"
L["Flow"] = "Поток"
L["Frame Name"] = "Название окна"
L["FrameNameDesc"] = "Отображаемое имя этого окна в панели настроек."
L["Frames"] = "Окна"
L["General"] = "Общее"
L["Item Loot"] = "Добыча предметов"
L["Loot Behavior"] = "Поведение добычи"
L["LootBehaviorDesc"] = "Управление стандартным поведением добычи Blizzard и перекрытие оповещений."
L["Manage Frames"] = "Управление окнами"
L["ManageFramesDesc"] = "Переименование или удаление окон отображения добычи."
L["MaxFramesReached"] = "Достигнут лимит в 5 окон."
L["Money"] = "Деньги"
L["New Frame Name"] = "Название нового окна"
L["NewFrameGroupDesc"] = "Создать новое окно для отображения добычи."
L["NewFrameNameDesc"] = "Введите название для нового окна и нажмите «Добавить окно»."
L["Profession Skills"] = "Профессиональные навыки"
L["Quick Actions"] = "Быстрые действия"
L["Reputation"] = "Репутация"
L["Row Text Spacing"] = "Межстрочный интервал текста"
L["RowTextSpacingDesc"] = "Расстояние в пикселях между элементами текста (ссылка на предмет, количество, индикатор счета). Установите 0 для автоподбора (зависит от размера иконки: iconSize / 4)."
L["Text Alignment"] = "Выравнивание текста"
L["TextAlignmentDesc"] = "Как содержимое строк выравнивается в ленте. При выборе «Слева» или «По центру» иконка будет слева; при выборе «Справа» - справа."
L["Transmog"] = "Трансмогрификация"
L["Travel Points"] = "Очки путешественника"
--#endregion

--#region 1.28.0
L["Unknown Faction"] = "Неизвестная фракция"
--#endregion

--#region 1.27.2
L["ClickToOpenCloakTree"] = "Нажмите на название предмета, чтобы открыть древо талантов плаща"
--#endregion

--#region 1.27.0
L["BackdropInsetsDesc"] = "Настройка отступа фона строки. Используйте это, если фон отображается на углах границ (при использовании текстурированных границ с закругленными краями)."
L["Background Texture"] = "Фоновая текстура"
L["Background Texture Color"] = "Цвет фоновой текстуры"
L["Background Type"] = "Тип фона"
L["BackgroundTextureColorDesc"] = "Цвет, который нужно применить к выбранной текстуре."
L["BackgroundTextureDesc"] = "Выберите текстуру фона из LibSharedMedia"
L["BackgroundTypeDesc"] = "Выберите тип фона для строк ленты добычи."
L["Border Texture"] = "Текстура границы"
L["BorderTextureDesc"] = "Выберите текстуру границы из LibSharedMedia"
L["Bottom Inset"] = "Нижний отступ"
L["BottomInsetDesc"] = "Фон будет сдвинут от нижнего края строки на это количество."
L["Gradient"] = "Градиент"
L["Left Inset"] = "Левый отступ"
L["LeftInsetDesc"] = "Фон будет сдвинут от левого края строки на это количество."
L["Right Inset"] = "Правый отступ"
L["RightInsetDesc"] = "Фон будет сдвинут от правого края строки на это количество."
L["Textured"] = "Текстурированный"
L["Top Inset"] = "Верхний отступ"
L["TopInsetDesc"] = "Фон будет сдвинут от верхнего края строки на это количество."
--#endregion

--#region 1.26.0
L["Hide All Icons"] = "Скрыть все иконки"
L["HideAllIconsDesc"] = "Если включено, все иконки будут скрыты в ленте добычи."
L["Show '1' Quantity"] = "Показывать количество '1'"
L["Show Currency Icon"] = "Показывать иконку валюты"
L["Show Experience Icon"] = "Показывать иконку опыта"
L["Show Item Icon"] = "Показывать иконку предмета"
L["Show Party Avatar"] = "Показывать аватары группы"
L["Show Profession Icon"] = "Показывать иконку профессии"
L["Show Reputation Icon"] = "Показывать иконку репутации"
L["Show Transmog Icon"] = "Показывать иконку трансмога"
L["Show Travel Point Icon"] = "Показывать иконку очков путешестий"
L["ShowCurrencyIconDesc"] = "Если включено, иконка валюты будет показана в ленте добычи."
L["ShowExperienceIconDesc"] = "Если включено, иконка опыта будет показана в ленте."
L["ShowItemIconDesc"] = "Если включено, иконка предмета будет показана в ленте добычи."
L["ShowOneQuantityDesc"] = "Если включено, количество 'x1' будет показано для единичной добычи. Если отключено, единичная добыча не будет показывать количество."
L["ShowPartyAvatarDesc"] = "Если включено, аватары группы будут показаны в ленте добычи."
L["ShowProfessionIconDesc"] = "Если включено, иконка профессии будет показана в ленте."
L["ShowRepIconDesc"] = "Если включено, иконка репутации будет показана в ленте."
L["ShowTransmogIconDesc"] = "Если включено, иконка новой модели трансмога будет показана в ленте добычи."
L["ShowTravelPointIconDesc"] = "Если включено, иконка очков путешествий будет показана в ленте добычи."
--#endregion

--#region 1.25.0
L["AtlasIconDescription"] = "Исходные данные ниже взяты из Atlas Texture, а не из путей к файлам. Вы можете использовать аддон TextureAtlasViewer для поиска действующих текстур Atlas. Размеры приведенных ниже иконок основаны на параметре размера вторичного шрифта. Пожалуйста, обратитесь ко мне, если размер иконки, который вы хотите, у вас не работает."
L["Auction House Icon Texture"] = "Иконка аукциона из Atlas Texture"
L["Auction Price then Vendor Price"] = "Цена на аукционе, затем у продавца"
L["AuctionHouseIconTextureDesc"] = "Текстура, которую можно использовать для иконки аукциона в ленте добычи. Это текстура Atlas, а не путь к файлу."
L["Chosen Icon"] = "Выбранная вами иконка: %s"
L["Enable Blizzard Transmog Sound"] = "Включить звук трансмога Blizzard"
L["Enable Transmog Effect"] = "Включить эффект трансмога"
L["Enable Transmog in Feed"] = "Включить трансмог в ленту"
L["EnableBlizzardTransmogSoundDesc"] = "Звук трансмога Blizzard будет воспроизведен, когда вы получаете новую модель трансмога."
L["EnableTransmogDesc"] = "Показывать новые модели трансмога в ленте добычи."
L["EnableTransmogEffectDesc"] = "Анимированная текстура будет отображаться на левой и правой сторонах ленты добычи при получении новой модели трансмога, аналогично эффекту уведомлений трансмога Blizzard."
L["Experimental"] = "Экспериментальный"
L["Highest Price"] = "Наибольшая цена"
L["Highlight New Transmog Items"] = "Подсвечивать новые модели трансмога"
L["Highlight Quest Items"] = "Подсвечивать предметы для заданий"
L["HighlightQuestDesc"] = "Подсвечивать предметы для заданий в ленте добычи."
L["HighlightTransmogDesc"] = "Подсвечивать модели трансмога, которых у вас ещё нет, в ленте добычи."
L["InGameCommunity"] = "Внутриигровое сообщество"
L["InGameCommunityDesc"] = "Присоединяйтесь к внутриигровому сообществу RPGlootFeed, чтобы получить помощь, обратную связь и внести предложения для комфортной игры! Скопируйте/вставьте код ниже, чтобы присоединиться к сообществу."
L["InvalidAtlasTexture"] = "%s не является действующей текстурой Atlas. Аддон TextUreAtlasViewer может помочь!"
L["Item Text Style Overrides"] = "Изменение стиля названий предметов"
L["Override Text Style for Quest Items"] = "Изменить стиль названий предметов  для заданий"
L["Play Sound for New Transmog Items"] = "Воспроизвести звук для новой модели трансмога"
L["PlaySoundForTransmogDesc"] = "Воспроизведите звук, когда вы получаете предмет с новой моделью трансмога."
L["Quest Item Color Text"] = "Цвет названий предметов для заданий"
L["QuestItemColorTextDesc"] = "Цвет названий предметов для заданий в ленте добычи. Это переопределит цвет качества ссылки."
L["QuestItemStyleOverrideDesc"] = "Стиль названий предметов для заданий будет изменен следующими настройками стиля текста предметов для заданий."
L["RevertAuctionHouseIconToDefaultDesc"] = "Вернуть текстуру иконки аукциона по умолчанию из Atlas Texture."
L["RevertVendorIconToDefaultDesc"] = "Вернуть текстуру иконки продавца по умолчанию из Atlas Texture."
L["Test Transmog Sound"] = "Проверить звук трансмога"
L["TestBetterThanEquippedSoundDesc"] = "Проверить выбранный звук для предметов, которые лучше экипированных"
L["TestLegendarySoundDesc"] = "Проверить выбранный звук для легендарных предметов"
L["TestMountSoundDesc"] = "Проверить выбранный звук для маунтов"
L["TestTransmogSoundDesc"] = "Проверить выбранный звук для новых моделей трансмога"
L["Transmog Config"] = "Настройка трансмога"
L["Transmog Options"] = "Опции трансмога"
L["Transmog Sound"] = "Звук трансмога"
L["TransmogSoundDesc"] = "Звук, который будет воспроизведен, когда вы получаете новую модель трансмога."
L["Vendor Icon Texture"] = "Иконка продавца из Atlas Texture"
L["Vendor Price then Auction Price"] = "Цена у продавца, затем на аукционе"
L["VendorIconTextureDesc"] = "Текстура, которую можно использовать для иконки продавца в ленте добычи. Это текстура Atlas, а не путь к файлу."
--#endregion

--#region 1.23.0
L["Only Income"] = "Показывать только доход"
L["OnlyIncomeDesc"] = "Показывать только доход (т.е. положительные суммы денег) в ленте добычи. Потраченные деньги, такие как ремонт, покупка товаров и т. д. не будут показаны."
L["Show Money Icon"] = "Показывать иконку перед деньгами"
L["ShowMoneyIconDesc"] = "Показывать иконку перед суммой денег в ленте добычи."
--#endregion

--#region 1.0.0 - 1.22.0
-- L["Ignore Item IDs"] = "Ignore Item IDs"
-- L["IgnoreItemIDsDesc"] = "Enter a comma-separated list of item IDs to ignore in the Party Loot Feed"
L["Abbreviate Total"] = "Сокращать общее количество"
L["AbbreviateTotalDesc"] = "Сокращать общее количество денег в ленте добычи (для золота свыше 1000)"
L["About"] = "О аддоне"
L["AboutDesc"] = "Об аддоне и его авторе"
L["AboutTitle"] = "Спасибо за использование RPGLootFeed!"
L["Accountant Mode"] = "Режим бухгалтера"
L["AccountantModeDesc"] = "Показывать отрицательные суммы денег в скобках вместо знака минус."
L["AddLootAlertUnavailable"] = "LootAlertSystem:AddAlert был недоступен более 30 секунд, уведомления о добыче не могут быть отключены :("
L["AddMoneyAlertUnavailable"] = "MoneyAlertSystem:AddAlert был недоступен более 30 секунд, уведомления о золоте не могут быть отключены :("
L["Alerts"] = "Уведомления"
L["Anchor Point"] = "Точка закрепления"
L["Anchor Relative To"] = "Закрепить относительно"
L["AnchorPointDesc"] = "Где на экране закрепить ленту добычи (также влияет на направление изменения размера)"
L["Angle Brackets"] = "<Угловые скобки>"
L["Animations"] = "Анимации"
L["AnimationsDesc"] = "Настройте анимации ленты добычи."
L["Artifact"] = "Артефакт"
L["Auction House Source"] = "Источник аукцион"
L["Auction Price"] = "Цена на аукционе"
L["Auctionator"] = "Auctionator"
L["AuctionHouseSourceDesc"] = "Выберите аддон для получения информации о ценах на аукционе"
L["Author"] = "Этот аддон создан и поддерживается McTalian."
L["AuthorGitHub"] = "GitHub (скопируйте/вставьте ниже)"
L["Background"] = "Фон"
L["Background Gradient End"] = "Конец градиента фона"
L["Background Gradient Start"] = "Начало градиента фона"
L["BagBar"] = "Панель сумок"
L["Bars"] = "|Полосы|"
L["Base Duration"] = "Базовая продолжительность"
L["BaseDurationDesc"] = "Количество секунд, которое займет анимация наведения."
L["Better Than Equipped Sound"] = "Звук для предметов лучше чем у вас"
L["BetterThanEquippedSoundDesc"] = "Звук, который проигрывается, когда получаете предмет, который лучше того, что у вас экипирован"
L["BillionAbbrev"] = "Млрд"
L["Blizzard UI"] = "Интерфейс Blizzard"
L["BlizzUIDesc"] = "Переопределить поведение элементов интерфейса Blizzard"
L["BossBannerAlertUnavailable"] = "BossBanner:OnEvent был недоступен более 30 секунд, элементы баннера босса не могут быть отключены :("
L["Bottom"] = "Низ"
L["Bottom Left"] = "Нижний левый"
L["Bottom Right"] = "Нижний правый"
L["Center"] = "Центр"
L["Clear rows"] = "Очистить строки"
L["Close"] = "Закрыть"
L["Common"] = "Обычный"
L["Credits"] = "%sОсобая благодарность всем участникам перевода!%s"
L["Curly Braces"] = "{Фигурные скобки}"
L["Currency Config"] = "Настройка валюты"
L["Currency messages Disabled"] = "Сообщения о валюте отключены"
L["Currency Options"] = "Опции валюты"
L["Currency Total Text Color"] = "Цвет текста общего количества валюты"
L["Currency Total Text Options"] = "Опции текста общего количества валюты"
L["Currency Total Text Wrap Character"] = "Символ обертывания текста общего количества валюты"
L["CurrencyTotalTextColorDesc"] = "Цвет текста общего количества валюты в ленте добычи."
L["CurrencyTotalTextWrapCharDesc"] = "Символы для обертывания текста общего количества валюты."
L["Current Level Color"] = "Цвет текущего уровня"
L["Current Level Options"] = "Опции текущего уровня"
L["Current Level Text Wrap Character"] = "Персонаж обертывания текста текущего уровня"
L["CurrentLevelColorDesc"] = "Цвет текста текущего уровня в ленте добычи."
L["CurrentLevelTextWrapCharDesc"] = "Персонажи для обертывания текста текущего уровня."
L["Custom Fonts"] = "Пользовательские шрифты"
L["CustomFontsDesc"] = "Настройте тип, размер и флаги шрифта для персонализации ленты добычи."
L["Default Rep Text Color"] = "Цвет текста репутации по умолчанию"
L["Dialog"] = "Диалог"
L["Disable All BossBanner"] = "Отключить весь баннер босса"
L["Disable All BossBanner Loot"] = "Отключить всю добычу баннера босса"
L["Disable Automatic Exit"] = "Отключить автоматическое скрытие"
L["Disable Boss Banner Elements"] = "Отключить элементы баннера босса"
L["Disable Currency Chat Messages"] = "Отключить сообщения о валюте в чате"
L["Disable Experience Chat Messages"] = "Отключить сообщения об опыте в чате"
L["Disable Highlight"] = "Отключить выделение"
L["Disable Loot Chat Messages"] = "Отключить сообщения о добыче в чате"
L["Disable Loot Toasts"] = "Отключить уведомления о добыче"
L["Disable Money Alerts"] = "Отключить уведомления о деньгах"
L["Disable Money Chat Messages"] = "Отключить сообщения о деньгах в чате"
L["Disable Party/Raid Loot"] = "Отключить добычу группы/рейда"
L["Disable Reputation Chat Messages"] = "Отключить сообщения о репутации в чате"
L["Disable Skill Chat Messages"] = "Отключить сообщения навыков"
L["DisableAutomaticExitDesc"] = "Если включено, лента добычи не будет автоматически удалять строки по истечении заданного времени. Для удаления нужно будет нажать ПКМ по строке."
L["DisableBossBannerDesc"] = "Переопределить баннер босса. Полностью скрыть его, скрыть только часть с добычей или только вашу добычу или добычу вашей группы."
L["DisableCurrencyChatMessagesDesc"] = "Отключает сообщения о валюте во всех окнах чата."
L["DisableExperienceChatMessagesDesc"] = "Отключает сообщения об опыте во всех окнах чата."
L["DisableHighlightDesc"] = "Если этот флажок установлен, граница строки не будет выделяться при повторном получении того же предмета и обновлении его количества."
L["DisableLootChatMessagesDesc"] = "Отключает сообщения о добыче во всех окнах чата."
L["DisableLootToastDesc"] = "Окна, которые появляются внизу экрана, когда вы добываете особые предметы"
L["DisableMoneyAlertsDesc"] = "Окна, которые появляются внизу экрана, когда вы получаете деньги, например, награды за мировые задания"
L["DisableMoneyChatMessagesDesc"] = "Отключает сообщения о деньгах во всех окнах чата."
L["DisableReputationChatMessagesDesc"] = "Отключает сообщения о репутации во всех окнах чата."
L["DisableSkillChatMessagesDesc"] = "Отключает сообщения о навыках/ремеслах/профессиях во всех чат-окнах."
L["Do not disable BossBanner"] = "Не отключать баннер босса"
L["Down"] = "Вниз"
L["Drag to Move"] = "Перетащите для перемещения"
L["Duration (seconds)"] = "%s Длительность показа"
L["DurationDesc"] = "Количество секунд отображения строки личного лута качества %s перед началом исчезновения. (0 для использования значения по умолчанию из настроек анимации)"
L["Enable Auto Loot"] = "Включить автоматический сбор добычи"
L["Enable Currency in Feed"] = "Включить валюту в ленту"
L["Enable Currency Total Text"] = "Включить текст общего количества валюты"
L["Enable Experience in Feed"] = "Включить опыт в ленту"
L["Enable Hover Animation"] = "Включить анимацию при наведении"
L["Enable Item Count Text"] = "Включить текст количества предметов"
L["Enable Item Loot in Feed"] = "Включить добычу предметов в ленту"
L["Enable Item/Currency Tooltips"] = "Включить подсказки для предметов/валюты"
L["Enable Loot History"] = "Включить историю добычи"
L["Enable Money in Feed"] = "Включить деньги в ленту"
L["Enable Party Loot in Feed"] = "Включить добычу группы в ленту"
L["Enable Professions in Feed"] = "Включить профессии в ленту"
L["Enable Reputation in Feed"] = "Включить репутацию в ленту"
L["Enable Reputation Level"] = "Включить уровень репутации"
L["Enable Row Borders"] = "Включить границы строк"
L["Enable Secondary Row Text"] = "Включить вторичный текст строки"
L["Enable Top Left Icon Text"] = "Включить текст в левом верхнем углу"
L["Enable Travel Points in Feed"] = "Показывать путешествия в ленте"
L["EnableAutoLootDesc"] = "Установить настройку по умолчанию, чтобы автоматический сбор добычи был включен при входе в игру на любом персонаже"
L["EnableCurrencyDesc"] = "Показывать валюту, такую как Камни полета, Честь, Пробужденный гребень дракона и т.д. в ленте добычи"
L["EnableCurrencyTotalTextDesc"] = "Показывать общее количество валюты на текущем персонаже в ленте добычи"
L["EnableHoverAnimationDesc"] = "Если этот флажок установлен, при наведении курсора на строку она будет выделена."
L["EnableItemCountTextDesc"] = "Показывать общее количество (в сумках, банке и т.д.) добытого предмета в ленте добычи"
L["EnableItemLootDesc"] = "Показывать добытые предметы в ленте добычи"
L["EnableLootHistoryDesc"] = "Сохранять историю добытых предметов и отображать их в отдельном окне"
L["EnableMoneyDesc"] = "Показывать деньги, такие как золото, серебро, медь, в ленте добычи"
L["EnablePartyLootDesc"] = "Показывать добытые предметы группы/рейда в ленте добычи"
L["EnableProfDesc"] = "Показывать повышение навыков профессий в ленте добычи"
L["EnableRepDesc"] = "Показывать получение репутации в ленте добычи"
L["EnableRepLevelDesc"] = "Показывать уровень репутации в ленте добычи."
L["EnableRowBordersDesc"] = "Если отмечено, показывать границы вокруг каждой строки в ленте добычи."
L["EnableSecondaryRowTextDesc"] = "Если отмечено, показывать вторичный текст строки, такой как уровень предмета, второстепенные характеристики, цена у продавца и т.д., если применимо."
L["EnableTooltipsDesc"] = "Включить отображение подсказок для предметов/валюты при наведении курсора, никогда не показывать в бою."
L["EnableTopLeftIconTextDesc"] = "Если включено, в левом верхнем углу иконок будет отображаться текст (например, уровень предмета для экипировки)."
L["EnableTravelPointsDesc"] = "Отображать путешествия (активности для получения Торговых жетонов) в ленте добычи"
L["EnableXPDesc"] = "Показывать получение опыта в ленте добычи"
L["Enter Animation Duration"] = "Длительность анимации появления"
L["Enter Animation Type"] = "Тип анимации появления"
L["EnterAnimationDurationDesc"] = "Количество секунд, которое займет анимация появления."
L["EnterAnimationTypeDesc"] = "Тип анимации, используемой при добавлении новой строки в ленту добычи."
L["Epic"] = "Эпический"
L["Exit Animation Duration"] = "Продолжительность анимации выхода"
L["Exit Animation Type"] = "Тип анимации выхода"
L["ExitAnimationDurationDesc"] = "Количество секунд, которое займет анимация выхода."
L["ExitAnimationTypeDesc"] = "Тип анимации, используемый при удалении строки из ленты добычи."
L["Experience Config"] = "Настройка опыта"
L["Experience Options"] = "Опции опыта"
L["Experience Text Color"] = "Цвет текста опыта"
L["ExperienceTextColorDesc"] = "Цвет текста опыта в ленте добычи."
L["Fade"] = "Исчезновение"
L["Fade Out Delay"] = "Задержка исчезновения"
L["FadeOutDelayDesc"] = "Количество секунд, в течение которых строка будет отображаться перед исчезновением."
L["Feed Width"] = "Ширина ленты"
L["FeedWidthDesc"] = "Ширина родительского фрейма ленты добычи"
L["Font"] = "Шрифт"
L["Font Face"] = "Тип шрифта"
L["Font Flags"] = "Флаги шрифтов"
L["Font Size"] = "Размер шрифта"
L["FontDesc"] = "Объект шрифта для текста добычи."
L["FontFaceDesc"] = "Стиль текста, который будет отображаться в ленте добычи."
L["FontFlagsDesc"] = "Флаги, применяемые к тексту ленты добычи."
L["FontSizeDesc"] = "Размер текста ленты добычи в \"пунктах\"."
L["Frame Strata"] = "Слой фрейма"
L["FrameStrataDesc"] = "Настройте слой (глубину экрана, z-индекс и т.д.) фрейма ленты добычи"
L["GitHubIssuesLink"] = "GitHub Issues (скопируйте/вставьте ниже)"
L["GradientEndDesc"] = "Конечный цвет градиента фона строки."
L["GradientStartDesc"] = "Начальный цвет градиента фона строки."
L["Grow Up"] = "Рост вверх"
L["GrowUpDesc"] = "Лента будет расти вверх (вниз, если не отмечено) по мере добавления новых предметов"
L["Heirloom"] = "Наследие"
L["Hide Loot History Tab"] = "Скрыть вкладку истории добычи"
L["Hide Server Names"] = "Скрывать названия серверов"
L["HideLootHistoryTabDesc"] = "Скрыть вкладку истории добычи, прикрепленную к ленте добычи"
L["HideServerNamesDesc"] = "Скрывать названия серверов в окне добычи группы"
L["High"] = "Высокий"
L["Highlight Items Better Than Equipped"] = "Подсвечивать предметы лучше экипированных"
L["Highlight Items with Tertiary Stats or Sockets"] = "Выделять предметы с дополнительными характеристиками или гнёздами"
L["Highlight Legendary Items"] = "Подсвечивать легендарные предметы"
L["Highlight Mounts"] = "Подсвечивать маунтов"
L["HighlightBetterThanEquippedDesc"] = "Подсвечивать предметы, которые лучше тех, что у вас надето, в ленте добычи"
L["HighlightLegendaryDesc"] = "Подсвечивать легендарные предметы в ленте добычи"
L["HighlightMountsDesc"] = "Подсвечивать маунтов в ленте добычи"
L["HighlightTertiaryOrSocketDesc"] = "Выделять предметы с дополнительными характеристиками или гнёздами в ленте добычи"
L["Hover Alpha"] = "Прозрачность наведения"
L["Hover Animation"] = "Анимация при наведении"
L["HoverAlphaDesc"] = "Альфа-канал строки подсвечивается при наведении курсора."
L["HoverAnimationDesc"] = "Настройте анимацию при наведении курсора на строку в ленте добычи."
L["IconSizeDesc"] = "Размер иконок в каждой строке ленты добычи"
L["Issues"] = "Пожалуйста, сообщите об этой проблеме на github: McTalian-WoW-Addons/RPGLootFeed"
L["IssuesAndRequests"] = "Вы также можете сообщить о проблемах или запросить новые функции на странице Issues в GitHub."
L["Item Count Text"] = "Текст количества предметов"
L["Item Count Text Color"] = "Цвет текста количества предметов"
L["Item Count Text Wrap Character"] = "Предмет Количество Текст Сворачивание Персонаж"
L["Item Highlights"] = "Подсветка предметов"
L["Item Loot Config"] = "Настройка добычи предметов"
L["Item Loot messages Disabled"] = "Сообщения о добыче предметов отключены"
L["Item Loot Options"] = "Опции добычи предметов"
L["Item Loot Sounds"] = "Звуки добычи предметов"
L["Item Quality Filter"] = "Фильтр качества предметов"
L["Item Secondary Text Options"] = "Опции вторичного текста предметов"
L["ItemCountTextColorDesc"] = "Цвет текста количества предметов в ленте добычи."
L["ItemCountTextWrapCharDesc"] = "Персонаж для обертывания текста количества предметов."
L["ItemHighlightsDesc"] = "Подсвечивать предметы в ленте добычи на основе определенных критериев"
L["ItemQualityFilterDesc"] = "Выберите, какие качества предметов показывать в ленте добычи."
L["JoinDiscord"] = "Присоединиться к сообществу Discord (скопируйте/вставьте ниже)"
L["JoinDiscordDesc"] = "Присоединяйтесь к сообществу RPGLootFeed в Discord для поддержки, отзывов, предложений и новостей о последних обновлениях."
L["LauncherLeftClick"] = "|cffeda55fНажать|r чтобы открыть меню конфигурации аддона."
L["LauncherRightClick"] = "|cffeda55fПКМ|r чтобы открыть быстрое меню."
L["Left"] = "Лево"
L["Legendary"] = "Легендарный"
L["Legendary Sound"] = "Легендарный звук"
L["LegendarySoundDesc"] = "Звук, который проигрывается при получении легендарного предмета"
L["Loop Update Highlight"] = "Выделение обновления цикла"
L["LoopUpdateHighlightDesc"] = "Если этот флажок установлен, анимация подсветки будет повторяться при обновлении количества."
L["Loot Feeds"] = "Лента добычи"
L["Loot History"] = "История Добычи"
L["Loot History Size"] = "Размер истории добычи"
L["Loot Item Height"] = "Высота строки добычи"
L["Loot Item Icon Size"] = "Размер иконки предмета добычи"
L["Loot Item Padding"] = "Отступ между строками добычи"
L["LootHistorySizeDesc"] = "Максимальное количество предметов для хранения в истории добычи"
L["Low"] = "Низкий"
L["Maximum Rows to Display"] = "Максимальное количество строк для отображения"
L["MaxRowsDesc"] = "Максимальное количество предметов добычи для отображения в ленте"
L["Medium"] = "Средний"
L["MillionAbbrev"] = "Млн"
L["Minimap"] = "Миникарта"
L["Money Config"] = "Настройка денег"
L["Money Loot Sound"] = "Звук добычи золота"
L["Money messages Disabled"] = "Сообщения о золоте отключены"
L["Money Options"] = "Опции денег"
L["Money Total Options"] = "Опции общего количества денег"
L["MoneyLootSoundDesc"] = "Замените стандартный звук добычи золота на собственный звук."
L["Monochrome"] = "Монохромный"
L["Mount Sound"] = "Звук средства передвижения"
L["MountSoundDesc"] = "Звук, который проигрывается при получении средства передвижения"
L["New version available"] = "Доступна новая версия: %s"
L["None"] = "Нет"
L["Only Disable My BossBanner Loot"] = "Отключить только мою добычу баннера босса"
L["Only Epic and Above in Instance"] = "Только эпическое и выше в подземелье"
L["Only Epic and Above in Raid"] = "Только эпическое и выше в рейде"
L["OnlyEpicAndAboveInInstanceDesc"] = "Показывать только эпические и выше предметы в ленте добычи группы в подземелье"
L["OnlyEpicAndAboveInRaidDesc"] = "Показывать только эпические и выше предметы в ленте добычи группы в рейде"
L["OnlyShiftOnEnterDesc"] = "Показывать подсказку только если удерживается Shift при наведении на предмет/валюту."
L["Outline"] = "Контур"
L["Override Money Loot Sound"] = "Переопределить звук добычи золота"
L["OverrideMoneyLootSoundDesc"] = "Если отмечено, использовать пользовательский звук для добычи золота."
L["Parentheses"] = "(Круглые скобки)"
L["Party Item Quality Filter"] = "Фильтр качества предметов группы"
L["Party Loot"] = "Добыча группы"
L["Party Loot Config"] = "Настройка добычи группы"
L["Party Loot Options"] = "Опции добычи группы"
L["PartyItemQualityFilterDesc"] = "Выберите, какие качества предметов показывать в ленте добычи группы."
L["Pending Items"] = "%s предметов в ожидании"
L["Play Sound for Items Better Than Equipped"] = "Воспроизвести звук для предметов, которые лучше, чем ваши"
L["Play Sound for Legendary Items"] = "Воспроизвести звук для легендарных предметов"
L["Play Sound for Mounts"] = "Воспроизвести звук для средства передвижения"
L["PlayerFrame"] = "Фрейм игрока"
L["PlaySoundForBetterDesc"] = "Воспроизводить звук, когда получаете предмет, который лучше того, что у вас экипирован"
L["PlaySoundForLegendaryDesc"] = "Воспроизвести звук при получении легендарного предмета"
L["PlaySoundForMountsDesc"] = "Воспроизвести звук при получении средства передвижения"
L["Poor"] = "Обычный"
L["Positioning"] = "Позиционирование"
L["PositioningDesc"] = "Позиционируйте и закрепите ленту добычи."
L["Prices for Sellable Items"] = "Цены для продаваемых предметов"
L["PricesForSellableItemsDesc"] = "Выберите, какую цену показывать для продаваемых предметов в ленте добычи"
L["Profession Config"] = "Настройка профессий"
L["Profession Options"] = "Опции профессий"
L["Rare"] = "Редкий"
L["RelativeToDesc"] = "Выберите фрейм для закрепления ленты добычи"
L["Rep messages Disabled"] = "Сообщения о репутации отключены"
L["RepColorDesc"] = "Цвет текста репутации по умолчанию в ленте добычи. Переопределяется цветами фракций."
L["RepLevelColorDesc"] = "Цвет текста уровня репутации в ленте добычи."
L["RepLevelWrapCharDesc"] = "Персонаж для обертывания текста уровня репутации."
L["Reputation Config"] = "Настройка репутации"
L["Reputation Level Color"] = "Цвет уровня репутации"
L["Reputation Level Options"] = "Опции уровня репутации"
L["Reputation Level Wrap Character"] = "Персонаж обертывания текста уровня репутации"
L["Reputation Options"] = "Опции репутации"
L["Reset All Duration Overrides"] = "Сбросить все переопределения длительности"
L["ResetAllDurationOverridesDesc"] = "Сбросить все переопределения длительности к значениям по умолчанию"
L["Right"] = "Право"
L["Row Border Color"] = "Цвет границы строки"
L["Row Border Thickness"] = "Толщина границы строки"
L["Row Borders"] = "Границы строк"
L["Row Enter Animation"] = "Анимация появления строки"
L["Row Exit Animation"] = "Анимация исчезновения строки"
L["RowBorderColorDesc"] = "Цвет границы строки."
L["RowBordersDesc"] = "Настройте границы строк."
L["RowBorderThicknessDesc"] = "Толщина границы строки."
L["RowEnterAnimationDesc"] = "Настройте анимацию появления строк в ленте добычи."
L["RowExitAnimationDesc"] = "Настройте анимацию исчезновения строк в ленте добычи."
L["RowHeightDesc"] = "Высота каждой строки в ленте добычи"
L["RowPaddingDesc"] = "Расстояние между строками в ленте добычи"
L["Screen"] = "Экран"
L["Secondary Font Size"] = "Размер вторичного шрифта"
L["Secondary Text Alpha"] = "Прозрачность вторичного текста"
L["SecondaryFontSizeDesc"] = "Размер вторичного текста в ленте добычи в \"пунктах\"."
L["SecondaryTextAlphaDesc"] = "Прозрачность вторичного текста репутации в ленте добычи."
L["Shadow Color"] = "Цвет тени"
L["Shadow Offset X"] = "Смещение тени по оси X"
L["Shadow Offset Y"] = "Смещение тени по оси Y"
L["ShadowColorDesc"] = "Цвет тени позади текста ленты добычи."
L["ShadowOffsetHelp"] = "Отрицательные значения для теней слева или снизу текста. -1 обычно желательно для размеров шрифта 8-10. Установите оба смещения на 0, чтобы отключить тень."
L["ShadowOffsetXDesc"] = "Горизонтальное смещение тени за текстом ленты добычи. Отрицательные значения для теней слева от текста. -1 обычно желательно для размеров шрифта 8-10. 0 для отключения горизонтальной тени."
L["ShadowOffsetYDesc"] = "Вертикальное смещение тени за текстом ленты добычи. Отрицательные значения для теней под текстом. -1 обычно желательно для размеров шрифта 8-10. 0 для отключения вертикальной тени."
L["Show Current Level"] = "Показывать текущий уровень"
L["Show Minimap Icon"] = "Показать значок на миникарте"
L["Show Money Total"] = "Показывать общее количество денег"
L["Show only when SHIFT is held"] = "Показывать только при удержании SHIFT"
L["Show Skill Change"] = "Показывать изменение навыков"
L["ShowCurrentLevelDesc"] = "Показывать ваш текущий уровень в строках получения опыта."
L["ShowMinimapIconDesc"] = "Показать значок RPGLootFeed на мини-карте"
L["ShowMoneyTotalDesc"] = "Показывать общее количество денег на текущем персонаже в ленте добычи"
L["ShowSkillChangeDesc"] = "Показывать изменение уровня навыков."
L["Sizing"] = "Размеры"
L["SizingDesc"] = "Настройте размеры ленты и ее элементов."
L["Skill Change Options"] = "Опции изменения навыков"
L["Skill messages Disabled"] = "Сообщения о навыках отключены"
L["Skill Text Color"] = "Цвет текста навыков"
L["Skill Text Wrap Character"] = "Персонаж обертывания текста навыков"
L["SkillColorDesc"] = "Цвет текста навыков в ленте добычи."
L["SkillTextWrapCharDesc"] = "Персонажи для обертывания текста навыков."
L["Slide"] = "Скольжение"
L["Slide Direction"] = "Направление скольжения"
L["SlideDirectionDesc"] = "Направление, в котором строка будет двигаться во время анимации скольжения."
L["Socket"] = "Гнездо"
L["Spaces"] = " Пробелы "
L["Square Brackets"] = "[Квадратные скобки]"
L["Styling"] = "Стили"
L["StylingDesc"] = "Настройте стиль ленты и ее элементов с помощью пользовательских цветов, выравнивания и т.д."
L["Test Mode Disabled"] = "Тестовый режим отключен"
L["Test Mode Enabled"] = "Тестовый режим включен"
L["Thick Outline"] = "Толстый контур"
L["ThousandAbbrev"] = "Тыс"
L["TipsLink"] = "Чаевые приветствуются, но не обязательны |T166311:0|t (скопируйте/вставьте ниже)"
L["Toggle Loot History"] = "Переключить историю добычи"
L["Toggle Test Mode"] = "Переключить тестовый режим"
L["Tooltip"] = "Подсказка"
L["Tooltip Options"] = "Опции подсказок"
L["Top"] = "Верх"
L["Top Left"] = "Верхний левый"
L["Top Left Icon Font Size"] = "Размер шрифта текста в углу иконки"
L["Top Left Icon Text Color"] = "Цвет текста в углу иконки"
L["Top Left Icon Text Options"] = "Настройки текста в левом верхнем углу иконок"
L["Top Right"] = "Верхний правый"
L["TopLeftIconFontSizeDesc"] = "Размер текста в левом верхнем углу иконок (в пунктах)."
L["TopLeftIconTextColorDesc"] = "Цвет текста в левом верхнем углу иконок."
L["Travel Point Options"] = "Настройки путешествий"
L["Travel Points Config"] = "Настройки путешествий"
L["Travel Points Text Color"] = "Цвет текста путешествий"
L["TravelPointsTextColorDesc"] = "Цвет текста, отображающего информацию о путешествиях в ленте добычи."
L["TSM"] = "TSM"
L["UIParent"] = "UIParent"
L["Uncommon"] = "Необычный"
L["Up"] = "Вверх"
L["Update Animation Duration"] = "Обновление продолжительности анимации"
L["Update Animations"] = "Обновление анимации"
L["UpdateAnimationDurationDesc"] = "Количество секунд, которое займет анимация обновления."
L["UpdateAnimationsDesc"] = "Настройте анимацию при обновлении количества в существующей строке в ленте добычи."
L["Use Class Colors for Borders"] = "Использовать цвета классов для границ"
L["Use Font Objects"] = "Использовать объекты шрифтов"
L["Use Quality Color"] = "Использовать цвет качества"
L["UseClassColorsForBordersDesc"] = "Если отмечено, использовать цвета классов для границ строк."
L["UseFontObjectsDesc"] = "Если отмечено, использовать объект шрифта для определения типа и размера шрифта."
L["UseQualityColorDesc"] = "Если включено, текст будет окрашиваться в цвет, соответствующий качеству предмета."
L["Vendor Price"] = "Цена у продавца"
L["Version"] = "Версия RPGLootFeed: %s"
L["View Notifications"] = "Просмотреть %d уведомлений"
L["Welcome"] = "Добро пожаловать! Используйте /rlf для просмотра настроек."
L["X Offset"] = "Смещение по X"
L["XOffsetDesc"] = "Настройте смещение ленты добычи влево (отрицательное) или вправо (положительное)"
L["XP"] = "Опыт"
L["XP messages Disabled"] = "Сообщения об опыте отключены"
L["Y Offset"] = "Смещение по Y"
L["YOffsetDesc"] = "Настройте смещение ленты добычи вниз (отрицательное) или вверх (положительное)"
L["You have Notifications"] = "У вас %d уведомлений"
--#endregion
