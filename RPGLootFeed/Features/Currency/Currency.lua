...
local function IsCurrencySupported()
	return Currency.isForever() or Currency.currencyApi.GetExpansionLevel() >= G_RLF.Expansion.WOTLK
end

local function UsesChatCurrencyEvents()
	return not Currency.isForever() and Currency.currencyApi.GetExpansionLevel() < G_RLF.Expansion.BFA
end

-- Trading Post (Blizzard_PerksProgram) is excluded from WoW Forever.
local function HasPerksProgram()
	return Currency.isRetail() and not Currency.isForever()
end
...
function Currency:OnDisable()
	if not IsCurrencySupported() then
		self:LogDebug("Disabled because currency system is unsupported", nil, nil, nil, "OnEnable")
		return
	end
	if UsesChatCurrencyEvents() then
