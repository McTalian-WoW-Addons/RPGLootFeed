...
local function createItemLootContextProvider()
	return function(context, data)
		...
		local function plainPrice(copper)
			...
			local parts = {}
			if g > 0 then
				local part = g .. ItemLoot.itemLootApi.GetGoldAmountSymbol()
			...
			if s > 0 or g > 0 then
				local part = s .. ItemLoot.itemLootApi.GetSilverAmountSymbol()
				if plainTextPricesColored then
					part = "|cffc7c7cf" .. part .. "|r"
				end
				table.insert(parts, part)
			end
			local copperPart = c .. ItemLoot.itemLootApi.GetCopperAmountSymbol()
			if plainTextPricesColored then
		...
		local function iconMarkup(icon)
			...
			return ItemLoot.itemLootApi.CreateAtlasMarkup(icon, iconSize, iconSize) .. " "
		end
		...
		if pricesForSellableItems == G_RLF.PricesEnum.Vendor and showVendorPrice then
			if plainTextPrices then
				context.secondaryText = iconMarkup(vendorIcon) .. plainPrice(vendorPrice * effectiveQuantity)
			else
				context.secondaryText = " "
			end
			return
		elseif pricesForSellableItems == G_RLF.PricesEnum.AH and showAuctionPrice then
			if plainTextPrices then
				context.secondaryText = iconMarkup(auctionIcon) .. plainPrice(auctionPrice * effectiveQuantity)
			else
				context.secondaryText = " "
			end
			return
		elseif pricesForSellableItems == G_RLF.PricesEnum.Highest then
			if showAuctionPrice or showVendorPrice then
				if plainTextPrices then
					...
					context.secondaryText = iconMarkup(icon) .. plainPrice(highestPrice * effectiveQuantity)
				else
					context.secondaryText = " "
				end
				return
...
function ItemLoot:BuildPayload(info, quantity, fromLink)
	...
	payload.secondaryTextFn = function(...)
		...
		-- output to "", which the row reads as "no secondary text", so restore
		-- the spacer whenever coin data is what's actually being displayed.
		if text == "" and payload.secondaryCoinDataFn and payload.secondaryCoinDataFn(existingQuantity) then
			return " "
		end
		return text
	end
